							...
							<div class="widget-main no-padding table-responsive">
								...
								id="tableimportchannel">
										<thead class="thin-border-bottom">
											<tr>
												<th><i class="icon-key icon-2x visible-xs blue"></i><span
											class="hidden-xs">Name</span></th>
												<th><i class="icon-globe icon-2x visible-xs  blue"></i><span
											class="hidden-xs">Url</span></th>
												<th><i class="icon-flag icon-2x visible-xs blue"></i><span
											class="hidden-xs">Type</span></th>
												<th class=" width-20"><i
											class="icon-adjust icon-2x visible-xs blue"></i><span
...
<jsp:include page="include/channel-shipping-mapping.jsp"></jsp:include>
	...
	<script type="text/javascript">
			...
			$(this).CRUD(
					{
						...
						success : function(data) {
							$('#channelselect').empty();
							$("<option/>").val("").html(
									"Select Channel").appendTo(
									$('#channelselect'));
							$.each(data, function() {
								$("<option/>").val(this.supportedChannelId)
										.html(this.channelName)
										.appendTo($('#channelselect'));
							});
						}
					});
			tableimportchannel = $('#tableimportchannel')
					.dataTable(
							{
								"bPaginate" : false,
								"bLengthChange" : false,
								...
								"aoColumns" : [
											...
											"mData" : function(channel) {
												for (var i = 0; i < channel.oimChannelAccessDetailses.length; i++) {
													if (channel.oimChannelAccessDetailses[i].oimChannelAccessFields.fieldId == 1) {
														...
														return '<a class="btn btn-default icon-info-sign btn-xs visible-xs addresspop" data-toggle="popover" data-container="body"  data-placement="bottom" data-content="'+channel.oimChannelAccessDetailses[i].detailFieldValue+'" data-original-title="Url"></a><div class="hidden-xs">'
										...
										},
										{
											...
											"mData" : function(channel) {
												...
												return '<a class="btn btn-yellow btn-minier radius-2 dropdown-hover"  '+onClick+'><i class="'+icon+'"></i> <span role="menu" class="dropdown-menu tooltip-success purple dropdown-menu dropdown-yellow pull-right dropdown-caret dropdown-close">'
														+ toolTip
														+ '</span></a>&nbsp;<a class="btn btn-success btn-minier radius-2 dropdown-hover" data-toggle="modal" href="#uploadordermodal" onclick="uploadFile($(this).parent().parent())"><i class="icon-upload"></i> <span data-rel="tooltip" class="dropdown-menu tooltip-success purple dropdown-menu dropdown-yellow pull-right dropdown-caret dropdown-close">File Upload</span></a>&nbsp;'
														+ '<a href="#singleordermodal" onclick="singleorder($(this).parent().parent())" class="btn btn-purple btn-minier radius-2 dropdown-hover" data-toggle="modal"><i class=" icon-plus-sign"></i> <span role="menu" class="dropdown-menu tooltip-success purple dropdown-menu dropdown-yellow pull-right dropdown-caret dropdown-close">Single Order Entry</span> </a>'
		...
		function del(e) {
			bootbox
					.confirm(
							...
							function(b) {
								if (b) {
									var channel = tableimportchannel
											.fnGetData(e[0]);
									$(this)
											.CRUD(
													{
		...
		function channel(cmd, e) {
			...
			resetChannelForm($('#channelselect').val());
																												...
																												.DataTable(
																														{
																															...
																															//"aaData" : data.oimOrderses.oimOrderDetailses,
																															"bDestroy" : true
																														})
																												.width(
																														'100%')
