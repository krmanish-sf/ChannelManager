...
include('includes/specialconfig.php');
...
function getOrders(){
	...
	if (mysql_num_rows($ordPendingQuery) > 0)
	{
		...
		while($res = mysql_fetch_array($ordPendingQuery))
		{
			...
			$xml_str .= "<products>";
			$prodsQuery = mysql_query("select op.products_model,op.final_price,op.products_price,op.products_quantity from orders_products op where op.orders_id=".$oID);
