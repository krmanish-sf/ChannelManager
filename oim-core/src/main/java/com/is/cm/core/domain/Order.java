...
public class Order extends DomainBase implements Serializable {
  ...
  public static Order from(OimOrders oimorder) {
    ...
    BeanUtils.copyProperties(oimorder, order,
        new String[] { "oimOrderBatches", "oimOrderDetailses" });
    order.oimOrderBatches = OrderBatch.from(oimorder.getOimOrderBatches());
    if (oimorder.getOimOrderDetailses() != null) {
      Iterator<OimOrderDetails> odIter = oimorder.getOimOrderDetailses().iterator();
    ...
    if (oimorder.getOimShippingMethod() != null) {
      order.setShippingMethod(ShippingMethod.from(oimorder.getOimShippingMethod()));
    }
    return order;
  }
  ...
  public OimOrders toOimOrder() {
    ...
    BeanUtils.copyProperties(this, oimorder,
        new String[] { "oimOrderBatches", "oimOrderDetailses", "shippingMethod" });
    oimorder.setOimOrderBatches(this.oimOrderBatches.toOimOrderBatches());
    if (this.shippingMethod != null)
      oimorder.setOimShippingMethod(this.shippingMethod.toOimShippingMethod());
    return oimorder;
  }
  ...
  public String getShippingAddress() {
    return String.format("%s %s %s %s %s %s", StringHandle.removeNull(deliveryStreetAddress),
        StringHandle.removeNull(deliverySuburb), StringHandle.removeNull(deliveryCity),
        StringHandle.removeNull(deliveryStateCode), StringHandle.removeNull(deliveryZip),
