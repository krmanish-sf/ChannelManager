...
public class Filter {
  ...
  public static char [] convertCharArrayToUnicode (char [] array) {
    StringBuffer sb = new StringBuffer(java.lang.reflect.Array.getLength(array));
//    int oldVal = 0;
    for (int i = 0; i < java.lang.reflect.Array.getLength(array); i++) {
      int val = (int) array[i];
      if (val < 7 || val == 11 || val == 12 || (val > 13 && val < 32) || val > 126 && val != 194 && val <= 256) {
...
//    /////////////////////////////
    ...
    if (filteredString.startsWith(", ")) {
      filteredString = filteredString.substring(2,filteredString.length());
    } else if (filteredString.startsWith(" , ")) {
      filteredString = filteredString.substring(3,filteredString.length());
    }
    filteredString = Filter.transForm("s/\\.\\./\\./gi",filteredString);
...
for(int i=0;i<datatofilter.length();i++){ // for reading one by one charactor from string
