...
public class FormObject {
	...
	public FormObject(String codeBase_, String file_, NVPair form_data_[],
			String desc_, String match_, String[] regMatches_, boolean print_,
	...
	public FormObject(String codeBase_, String file_, String desc_,
	...
	public FormObject(String codeBase_, int port_, String file_, String desc_,
	...
	public FormObject(String codeBase_, String file_, NVPair form_data_[],
			String desc_, String match_, boolean print_, boolean store_,
	...
	public Hashtable hitForm(String reqType, String uploadFile) {
		...
		try {
			if (print) {
				printThis("<font color='blue'>Attempting " + desc
						+ "</font><br>\n");
			}
			if (store)
			...
			try {
				...
				if (addAuthorization) {
					authString = "Basic "
							+ Base64.encodeString(authUsername + ":"
									+ authPassword);
					con.addBasicAuthorization(authRealm, authUsername,
							authPassword);
				}

				NVPair[] defaultHeaders = new NVPair[] {
						new NVPair("User-Agent", this.userAgent),
						new NVPair("Authorization", authString) };
				if (from_url.length() > 0) {
					defaultHeaders = new NVPair[] {
							new NVPair("User-Agent", this.userAgent),
				...
				if (addExtraHeaders) {
					int defaultHeadersLength = java.lang.reflect.Array
							.getLength(defaultHeaders);
					int extraHeadersLength = java.lang.reflect.Array
							.getLength(this.extraHeaders);

					NVPair[] tempHeaders = defaultHeaders;
					defaultHeaders = new NVPair[defaultHeadersLength
							+ extraHeadersLength];
					for (int z = 0; z < java.lang.reflect.Array
							.getLength(defaultHeaders); z++) {
						if (z < defaultHeadersLength) {
							defaultHeaders[z] = tempHeaders[z];
						} else {
							defaultHeaders[z] = this.extraHeaders[z
									- defaultHeadersLength];
						}
					}
				}
				...
				/*
				 * if (this.timeOut > 0) { fot = new FormObjectTimeout(con, rsp,
				 * this.timeOut); fot.start(); }
				 */
				...
				if ("PostFile".equals(reqType)) {
					...
					if (uploadFile != null) {
						...
						byte[] data = Codecs.mpFormDataEncode(opts, thefile,
								hdrs);
			...
			if (printBytes) {
				try {
					...
					for (int in = 0; in < java.lang.reflect.Array
							.getLength(bytes); in++) {
			...
			if (statusCode == 400
					&& rsp.toString().indexOf("Effective-URI") != -1) {
				String effective = "";
				Regex uri = Regex.perlCode("/Effective-URI: (.+?)\\n/gi");
				if (uri.search(rsp.toString())) {
					effective = uri.stringMatched(1).substring(0,
							uri.stringMatched(1).length() - 1);
				}
			...
			// System.out.println("\n"+"!!! REDIRECTING TO:\n"+Location);
				...
				FormObject formObj = new FormObject(codeBase_, file_,
						"inside FormObject redirecting...", "", regMatches,
			...
			if (statusCode >= 300 && statusCode > 0 && go) {
				...
				// if (print) printThis("<font color='blue'>" + new
				// String(rsp.getData()) + "</font><br>\n");
				...
				if (saveFile) {
					...
					if ((file.toLowerCase().endsWith(".zip")) || zipFlag) {
						...
						ZipInputStream zipped_in = new ZipInputStream(
								rsp.getInputStream());
						ZipOutputStream zipped_out = new ZipOutputStream(
								fileOut);
						zipped_out.setMethod(ZipOutputStream.DEFLATED);
						byte[] b = new byte[1000];
						int len = -1;
						while (true) {
							// Get the next zip entry. Break out of the loop if
							// there are no more.
							ZipEntry zipentryIN = zipped_in.getNextEntry();
							if (zipentryIN == null)
								break;
							ZipEntry zipentry = new ZipEntry(
									zipentryIN.getName()); // this is a fix
							System.out.println("\n" + "New ZipEntry!");
				...
				while (keepGoing && (inputLine = in.readLine()) != null) {
					...
					for (i = 0; i < length; i += 2) {
						...
						if (temp.search(inputLine)) {
							...
							int absInd = Math
									.abs(new Integer(regMatches[i + 1])
											.intValue());
							for (j = 1; j <= absInd; j++) {
								if (temp.stringMatched(j) instanceof String) {
									matches.put(
											new Integer(i / 2 + 1).toString()
													+ ":"
													+ new Integer(j).toString(),
											temp.stringMatched(j));
									if (new Integer(regMatches[i + 1])
											.intValue() < 0) {
										if (!getPage) {
											keepGoing = false;
										}
				...
				if (check) {
					if (print) {
						printThis("<font color='blue'>" + desc
								+ " Successful!</font><br>\n");
					}
					if (store)
			...
			if (print)
				printThis("<font color='blue'>" + err + "</font><br>\n");
			if (store)
				error += err + "\n";
			okay = false;
			return matches;
			...
			ExcHandle.printStackTraceToErr(e);
			if (print)
				printThis("<font color='blue'>"
						+ ExcHandle.getStackTraceAsString(e) + "</font><br>\n");
			if (store)
