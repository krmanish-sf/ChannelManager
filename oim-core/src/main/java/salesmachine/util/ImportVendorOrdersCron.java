...
public class ImportVendorOrdersCron {
	...
	public static Vector getActiveOIMVendor(Session dbSession){
		...
		try {
			...
			Query query = dbSession.createQuery("from salesmachine.hibernatedb.Reps r where r.cmAllowed=:cmAllowed");
			query.setString("cmAllowed", "1");
			Iterator it = query.iterate();
			while (it.hasNext()) {
				r = (salesmachine.hibernatedb.Reps) it.next();
				activeVendors.add(r.getVendorId());
				System.out.println("Active Channel Manager VenderID : "+r.getVendorId());
			} 
			tx.commit(); 
	...
	private static void getChannels( Integer vid){
		...
		Query query = session.createQuery("select distinct c from salesmachine.hibernatedb.OimChannels c " +
		...
		List channels = query.setInteger("vid", vendors.getVendorId().intValue()).list();
		for(int i = 0; i<channels.size();i++){
			OimChannels channel = (OimChannels)channels.get(i);
			System.out.println("Active VID : "+vid +" with Channel ID :  "+channel.getChannelId()+"   Channel Name : "+channel.getOimSupportedChannels().getChannelName());
	...
	private static void startOrderPullForChannel(int channelId){
		Session session = SessionManager.currentSession();
		Query query = session.createQuery("from salesmachine.hibernatedb.OimChannels as c where c.channelId=:channelID");
		...
		System.out.println("Supported channel : "+channel.getOimSupportedChannels().getChannelName());
		...
		if (coi != null) {
			...
			if (! coi.init(channelId, session, null)) {
				System.out.println("Failed initializing the channel with channelId:"+channelId);
			} else {
				System.out.println("Pulling orders for channel id: " + channelId);			
				coi.getVendorOrders();
			}					
		} else {
			System.out.println("ERROR - Could not find a bean to work with this market. ");
		}
	}
}
