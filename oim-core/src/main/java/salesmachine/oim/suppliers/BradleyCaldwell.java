...
public class BradleyCaldwell extends Supplier {
	...
	public void sendOrders(Integer vendorId, OimVendorSuppliers ovs, List orders) {		
		...
		String dtl_uploadfilename = "dtl_"+accountNumber+"_"+sdf.format(new Date())+".txt";
		String hdr_uploadfilename = "hdr_"+accountNumber+"_"+sdf.format(new Date())+".txt";
		...
		try {
			...
			dtl_generateCsvFile(orders,dtl_getFileFieldMap(), dtl_uploadfilename, fileFormatParams, new StandardFileSpecificsProvider(session,ovs,v));						

			logStream.println("Generating file "+hdr_uploadfilename);
			hdr_generateCsvFile(orders,hdr_getFileFieldMap(), hdr_uploadfilename, fileFormatParams, new StandardFileSpecificsProvider(session,ovs,v), 
					accountNumber);						
	...
	private List dtl_getFileFieldMap() {
		...
		String fields[] = {"Order Number", "Line Number", "Item Number", "Description",
				"Quantity to Order", "SoldUOM", "STDUOM", "ALTUOM"};
		Integer mappedFieldIds[]= {2,0,1,24,
				9,0,0,0};		
	...
	private List hdr_getFileFieldMap() {
		...
		String fields[] = {"Order Number", "Customer Number", "Ship To", "Order Date ", "Purchase Order Text",
				"Ship By", "Name", "Add1", "Add2", "City",
				"State", "Zip", "Affil"};
		Integer mappedFieldIds[]= {2,0,0,34,2,
				0,3,4,0,5,
				6,7,0};		
		for (int i=0;i<fields.length;i++) {
			OimFields field = new OimFields(fields[i],fields[i],new Date(),null,null);
	...
	public void dtl_generateCsvFile(List orders, List fileFieldMaps, String fileName, Hashtable fileFormatParams, 
				...
				for(Iterator it=fileFieldMaps.iterator();it.hasNext();) { 
					...
					if(j == 0){
						...
						if(defaultLength > fieldValueLength){
							int appendDefault = defaultLength - fieldValueLength;
	...
	public void hdr_generateCsvFile(List orders, List fileFieldMaps, String fileName, Hashtable fileFormatParams, 
			IFileSpecificsProvider fileSpecifics, String accountNumber) throws Exception {
		FileWriter outputFile = new FileWriter(fileName);
