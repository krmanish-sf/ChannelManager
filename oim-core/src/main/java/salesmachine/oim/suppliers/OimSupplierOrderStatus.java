...
public class OimSupplierOrderStatus {
	...
	public boolean fetchOrderStatus() {
		...
		Query query = m_dbSession
				.createQuery("select m from OimSupplierMethods as m inner join m.oimSupplierMethodattrValueses v where m.deleteTm is null and m.oimSuppliers=:supp and m.oimSupplierMethodTypes.methodTypeId=:methodTypeId and v.deleteTm is null");
		query.setEntity("supp", m_supplier);
		query.setInteger("methodTypeId",
				OimConstants.SUPPLIER_METHOD_TYPE_STATUSPULL.intValue());
		...
		query = m_dbSession
				.createQuery("select distinct c.vendorId from salesmachine.hibernatedb.OimChannels as c, "
		...
		query.setEntity("status", new OimOrderStatuses(
				OimConstants.ORDER_STATUS_PLACED));
		tx.commit();
	...
	private boolean fetchVendorOrderStatus(Integer vendorId) {
		...
		Transaction tx = m_dbSession.beginTransaction();
		Query query = m_dbSession
				.createQuery("from OimVendorSuppliers s where s.oimSuppliers=:supp and s.vendorId=:vid and s.deleteTm is null");
		query.setEntity("supp", m_supplier);
		...
		if (OimConstants.SUPPLIER_METHOD_NAME_FTP.equals(supplierMethodNameId)) {
			...
			query = m_dbSession
					.createQuery("from OimFileformatParams where fileTypeId=:fileId and deleteTm is null");
			...
			while (it.hasNext()) {
				...
				fileFormatParams.put(param.getParamName(),
						param.getParamValue());
			}

			String remoteFile = (String) fileFormatParams
			...
			String localFileName = "" + vendorId + "-"
					+ m_supplier.getSupplierId() + ".txt";
			...
			List statusData = readStatusFile(oimFile, localFileName,
					fileFormatParams);
			...
			if (statusData != null && statusData.size() > 0) {
				for (int i = 0; i < statusData.size(); i++) {
					...
					String purchaseOrderNumber = (String) values
							.get(OimConstants.OIM_FIELD_PRODUCT_ORDER_NUMBER);
					String sku = (String) values
							.get(OimConstants.OIM_FIELD_SKU);
					String detailStatus = (String) values
							.get(OimConstants.OIM_FIELD_SUPPLIER_DETAIL_STATUS);

					Integer status = null;
					if ("C".equalsIgnoreCase(detailStatus)) {
						System.out.println("Order Id: " + purchaseOrderNumber
								+ "\tSKU:" + sku + "\tOrder Completed");
						status = OimConstants.ORDER_STATUS_PROCESSED_SUCCESS;
					} else if ("B".equals(detailStatus)) {
						System.out.println("Order Id: " + purchaseOrderNumber
								+ "\tSKU:" + sku + "\tBack Ordered");
						...
						System.out.println("Order Id: " + purchaseOrderNumber
								+ "\tSKU:" + sku + "\tOrder Pending");
						status = OimConstants.ORDER_STATUS_PROCESSED_PENDING;
					} else {
						System.out.println("Unknown detail status: '"
								+ detailStatus + "'");
						continue;
					}

					query = m_dbSession
							.createQuery("from OimOrderDetails d where d.sku=:sku and d.deleteTm is null and d.oimOrders.orderId=:oid and d.oimOrderStatuses.statusId in (:status1,:status2)");
					query.setString("sku", sku);
					query.setString("oid", purchaseOrderNumber);
					query.setInteger("status1",
							OimConstants.ORDER_STATUS_PLACED.intValue());
					query.setInteger("status2",
							OimConstants.ORDER_STATUS_PROCESSED_PENDING
									.intValue());
					OimOrderDetails detail = (OimOrderDetails) query.iterate()
							.next();
	...
	public static void main(String[] args) {
		if (args.length == 1) {
			...
			System.out.println("Processing orders for SupplierId: "
					+ supplierId);
