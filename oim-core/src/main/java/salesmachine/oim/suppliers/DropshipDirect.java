...
public class DropshipDirect extends Supplier {
	...
	public void sendOrders(Integer vendorId, OimVendorSuppliers ovs, List orders){		
		...
		String uploadfilename = accountNumber+"-"+sdf.format(new Date())+".xls";
		...
		try {
			OimSupplierOrderPlacement.generateXlsFile(orders,getFileFieldMap(), uploadfilename, fileFormatParams, new StandardFileSpecificsProvider(session,ovs,v));
