...
public class Mobileline extends Supplier {
	...
	public void sendOrders(Integer vendorId, OimVendorSuppliers ovs,List orders){		
		...
		if (shippingMethods.size() == 0) {
			logStream.println("No shipping mapping set. Can not process orders. ");
		}
        ...
        //String requestHeader = "apiuser="+StringHandle.removeNull(ovs.getLogin())+"&apikey="+StringHandle.removeNull(ovs.getPassword())+"&customer_number="+StringHandle.removeNull(ovs.getAccountNumber())+"&method=cconfile";
		String requestHeader = "apiuser="+StringHandle.removeNull(ovs.getLogin())+"&apikey="+StringHandle.removeNull(ovs.getPassword())+"&customer_number="+StringHandle.removeNull(ovs.getAccountNumber())+"&method=purchaseorder";
        String data = "";
        String answer=null;
        ...
        for(int i=0;i<orders.size();i++){        
        	...
        	String customer_notes = StringHandle.removeNull(order.getOrderComment());
        	if (customer_notes.length() == 0 && shippingMethodCode.toLowerCase().equalsIgnoreCase("customeraccountmodule_customeraccount")) {
        		logStream.println("Adding customer_notes: "+shippingDetails);
        		customer_notes = shippingDetails;
        	}
			if (shippingMethodCode.length() == 0) {
				logStream
						.println(shippingDetails+": Couldnt find the shipping code | Assigning the default shipping : "
								+ ovs.getDefShippingMethodCode());
				shippingMethodCode = StringHandle.removeNull(ovs.getDefShippingMethodCode());
			}else{
				logStream.println("!!! Shipping method code found : "+shippingMethodCode);
			}
			
			if(shippingMethodCode==null||shippingMethodCode.trim().length()==0){
				shippingMethodCode="";
				logStream.println("Shipping method code couldnt be found as defined in the order shipping details and also the default shipping code was not set.");
			}
			
			String countryId = order.getDeliveryCountry();
			if(countryId!=null&& countryId.trim().length()>2){
				countryId = StringHandle.removeNull((String)countryCodeMapping.get(countryId.trim()));
			}
        	...
        	data ="";
        	data += requestHeader;
        	data += "&po_number="+StringHandle.removeNull(order.getStoreOrderId())+"&customer_note="+URLEncoder.encode(customer_notes);
			
			data += "&shipping_method="+URLEncoder.encode(shippingMethodCode);
			data += "&firstname="+URLEncoder.encode(StringHandle.removeNull(order.getDeliveryName()))+"&lastname=&company="+URLEncoder.encode(StringHandle.removeNull(order.getDeliveryCompany()));
			data += "&street="+URLEncoder.encode(StringHandle.removeNull(order.getDeliveryStreetAddress()))+" "+URLEncoder.encode(StringHandle.removeNull(order.getDeliverySuburb()));
			data += "&city="+URLEncoder.encode(StringHandle.removeNull(order.getDeliveryCity()))+"&country_id="+URLEncoder.encode(StringHandle.removeNull(countryId));
			data += "&region="+URLEncoder.encode(StringHandle.removeNull(order.getDeliveryState()))+"&postcode="+URLEncoder.encode(StringHandle.removeNull(order.getDeliveryZip()));
			
			String telephone = StringHandle.removeNull(order.getDeliveryPhone());
			if (telephone.length() == 0)
				telephone = "1";
			data += "&telephone="+URLEncoder.encode(telephone)+"&email="+URLEncoder.encode(StringHandle.removeNull(order.getDeliveryEmail()));
			
				String productsStr = "";
			
			String supplierPrefix = (String)orderSkuPrefixMap.get(order.getOimOrderBatches().getOimChannels().getChannelId());
			...
			try {
	            // Send the request
	            URL url = new URL("http://host2.authsafe.com/~bamadmin/inventorysource/oim/mobileline/MobileLineApi.php");
	            ...
	            if(returnData!=null && returnData.toString().trim().length()>84){  //length 84 is to make sure all the tags '<request>','</request>','<response>' etc are coming
	            	soapRequest = returnData.substring(returnData.indexOf("<request>")+9, returnData.indexOf("</request>"));
	            	logStream.println("-------------- XML SOAP REQUEST SENT -------------\n");
	            	logStream.println(soapRequest+"\n");

	            	soapResponse = returnData.substring(returnData.indexOf("<response>")+10, returnData.indexOf("</response>"));
	            	logStream.println("-------------- XML SOAP RESPONSE -------------\n");
	            	logStream.println(soapResponse+"\n");

	            	answer = returnData.substring(returnData.indexOf("<result>")+8, returnData.indexOf("</result>"));
	            }else{
	            	logStream.println("!! Blank response came from the supplier side");
	            }
	            //Output the response
	            if (answer.indexOf("Create New Order")!=-1) {
					...
					updateVendorSupplierOrderHistory(vendorId, ovs, answer.toString());
				}
	     ...
	     // Send Email Notifications if is set to true.
			...
			if (answer !=null) {
				...
				logStream.println("!! ORDER PLACED : "
						+ ((answer.indexOf("Create New Order") != -1)==true?"Yes":"No"));
	...
	/***
	 * 
	 * @return Hashmap containing the shipping info for ML
	 * USPS First-Class Mail: usps_First-Class Mail Package
	 * USPS Parcel Post: usps_Parcel Post
	 * USPS Priority Mail: usps_Priority Mail
	 * USPS Express Mail: usps_Express Mail

	 * Customer Account: customeraccountmodule_customeraccount
	 * (note: you will need to add your shipping method in the comments if you
	 * are shipping through Terry's UPS account)

	 * Federal Express Ground: fedex_FEDEXGROUND 
	 * Federal Express Express Saver: fedex_FEDEXEXPRESSSAVER 
	 * Federal Express 2Day: fedex_FEDEX2DAY 
	 * Federal Express Standard Overnight: fedex_STANDARDOVERNIGHT 
	 * Federal Express Priority Overnight: fedex_PRIORITYOVERNIGHT 
	 * Federal Express First Overnight: fedex_FIRSTOVERNIGHT 
	 * 
	 * UPS Ground: ups_03 
	 * UPS Three-Day Select: ups_12 
	 * UPS Second Day Air: ups_02 
	 * UPS Second Day Air: A.M. ups_59 
	 * UPS Next Day Air: Saver ups_13 
	 * UPS Next Day Air: ups_01 
	 * UPS Next Day Air: Early A.M. ups_14
	 * 
	 */
	...
	private HashMap  createShippingMethods(){
		...
		fedex.put("fedex_STANDARDOVERNIGHT", "Federal Express Standard Overnight");
		fedex.put("fedex_PRIORITYOVERNIGHT", "Federal Express Priority Overnight");
		...
		customerAccoutFee.put("customeraccountmodule_customeraccount", "Customer Account Fee");
		
		HashMap onTrackShipping = new HashMap();
		onTrackShipping.put("regionalfreeshipping_regionalfreeshipping", "OnTrac Shipping");
