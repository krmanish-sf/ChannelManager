...
public class AutomationManager {
  ...
  public static void main(String[] args) {
    if (args != null && args.length > 0 && "stopped".equalsIgnoreCase(args[0])) {
      sendNotification();
    }
    ...
    ExecutorService executor = Executors
        .newFixedThreadPool(ApplicationProperties.getAutotmationThreadPoolSize());
    ...
    timer.schedule(orderPullTask, DELAY, ApplicationProperties.getOrderPullInterval());
    TimerTask orderTrackingTask = new OrderTrackingTask(eventBus, audit);
    timer.schedule(orderTrackingTask, DELAY, ApplicationProperties.getOrderTrackingInterval());
