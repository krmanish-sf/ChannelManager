...
public class SaveAutomationAudit {
  ...
  private static final Logger log = LoggerFactory.getLogger(SaveAutomationAudit.class);
    ...
    if (isPullTaskCompleted && isTrackTaskCompleted) {
      ...
      try {
        for (Iterator<Integer> itr = channelSet.iterator(); itr.hasNext();) {
          ...
          automationAudit
              .setTotalOrderPull(pullMap.get(channelId) != null ? pullMap.get(channelId) : 0);
          automationAudit
              .setTotalOrderTracked(trackMap.get(channelId) != null ? trackMap.get(channelId) : 0);
        ...
        if (tx != null && tx.isActive())
          tx.rollback();
        log.error("Exception occured while saving Automation Audit", e);
