...
public class ChannelEventHandler implements ChannelService {
	private final static Logger LOG = LoggerFactory.getLogger(ChannelEventHandler.class);
	...
	@Override
	public ChannelDetailsEvent getChannel(RequestChannelDetailEvent requestChannelDetailEvent) {
		OimChannels channel = channelRepository.findById(requestChannelDetailEvent.getId());
		return channel == null ? ChannelDetailsEvent.notFound(requestChannelDetailEvent.getId())
				: new ChannelDetailsEvent(requestChannelDetailEvent.getId(), Channel.from(channel));
	}

	@Override
	public ChannelDeletedEvent deleteChannel(DeleteChannelEvent deleteChannelEvent) {
		channelRepository.delete(deleteChannelEvent.getId());
		// FIXME::
		return new ChannelDeletedEvent(deleteChannelEvent.getId(), new Channel());
	}

	@Override
	public ReadEvent<String> pullOrders(ReadEvent<Channel> readEvent) {
		return new ReadEvent<String>(readEvent.getId(), pullOrders(readEvent.getId()));
	}

	@Override
	public UpdatedEvent<Channel> update(UpdateEvent<Map<String, String>> event) {
		Channel entity = channelRepository.save(event.getId(), event.getEntity());
		return new UpdatedEvent<Channel>(entity.getChannelId(), entity);
	}

	@Override
	public CreatedEvent<Channel> create(CreateEvent<Map<String, String>> createEvent) {
		Channel channel = channelRepository.findByName(createEvent.getEntity().get("channelname"));
		if (channel != null) {
	...
	public ReadEvent<List<Filetype>> getFileTypes(ReadEvent<Integer> event) {
		List<Filetype> fileTypes = channelRepository.getFileTypes(event.getId());
		return new ReadEvent<List<Filetype>>(event.getId(), fileTypes);
	}

	@Override
	public ReadEvent<List<UploadedFile>> getUploadedFiles(ReadEvent<Integer> event) {
		List<UploadedFile> fileTypes = channelRepository.getOimUploadedFiles(event.getId());
		return new ReadEvent<List<UploadedFile>>(event.getId(), fileTypes);
	}

	@Override
	public ReadEvent<String> pullOrders(RequestReadEvent<Channel> requestReadEvent) {
	...
	private String pullOrders(int channelId) {
		...
		OimOrderBatchesTypes oimOrderBatchesTypes = new OimOrderBatchesTypes(OimConstants.ORDERBATCH_TYPE_ID_MANUAL);
		try {
			...
			oimOrderBatches.setErrorCode(ChannelCommunicationException.getErrorcode());
		} catch (ChannelCommunicationException e) {
			LOG.error("Error in pulling orders for channel id: {}", channelId, e);
			stream.println(e.getMessage());
			oimOrderBatches.setDescription(
					"Error occured in pulling order due to ChannelComunication Error." + e.getMessage());
			oimOrderBatches.setErrorCode(ChannelCommunicationException.getErrorcode());
	...
	@Override
	public ReadCollectionEvent<ChannelShippingMap> findShippingMethods(RequestReadEvent<Integer> requestReadEvent) {
	...
	public ReadCollectionEvent<SupportedChannel> getSupportedChannels(
		...
		List<SupportedChannel> supportedChannels = channelRepository.findSupportedChannels();
		return new ReadCollectionEvent<SupportedChannel>(supportedChannels);
	}

	@Override
	public ReadEvent<Map<String, String>> getBigcommerceAuthDetailsByUrl(RequestReadEvent<String> requestReadEvent) {
	...
	@Override
	public ReadEvent<Map<String, String>> getShopifyAuthDetailsByUrl(RequestReadEvent<String> requestReadEvent) {
		Map<String, String> authDetails = channelRepository
				.findShopifyAuthDetailsByUrl(requestReadEvent.getEntity());
		return new ReadEvent<Map<String, String>>(0, authDetails);
	}
}
